...
<input type="hidden" id="hidScroll" name="a">
	...
	<h1 style="margin-top: 50px;margin-left: 20px;font-family: 'Krona One', sans-serif;font-size: 2.5vw;font-weight: 1;display: inline-block;">Feed</h1>

	<form class="example" action="search.php" method="post" style="margin-left:60%;max-width:300px;max-height: 50px;display: inline-block;">
  <input type="text" placeholder="Enter a keyword" name="search2" style="margin-top: 39px" required>
  <button type="submit" style="margin-left:40%;float: right;margin-top: -31px;"><i class="fa fa-search"></i></button>
</form>


<div class="qpanel" id="qpanelscroll" onscroll="fScroll(this);">
	<?php
	$sql="SELECT * FROM question ORDER BY qdate DESC";
	...
	echo "<span id='qstyle'>"."There are no questions to display"."</span>";
	...
	function fScroll(val)
{
		...
		localStorage.setItem("someVarKey", scrolval);
}

function fScrollMove(what)
{
        var hidScroll = document.getElementById('hidScroll');
		hidScroll.value = localStorage.getItem("someVarKey");
